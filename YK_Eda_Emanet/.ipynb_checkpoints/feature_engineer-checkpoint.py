...
class FeatureEngineer:
    ...
    def preprocess_validation_data(self, data):
        data.loc[:, 'age'] = data['age'].fillna(self.median_age)
        data.loc[:, 'age-hours'] = data['age'] * data['hours-per-week']
        data = self.fill_missing_education(data, self.occupation_to_education)
        data.loc[:, "education_level_encoded"] = self.ordinal_encoder.transform(data[["education_level"]])
        data.loc[:, 'education_work'] = data["education_level_encoded"] * data['hours-per-week']
